...
export class PingConnector implements PingObserver {
    ...
    async Notify(status: PingResult): Promise<void> {
        ...
        this.sinks.forEach(async (s) => s.Flush(await this.ReplaceKeywords(status.isAlive ? this.options.onAliveTemplate : this.options.onDeadTemplate, elapsed)));
    }

    private ReplaceKeywords(str: string, elapsed: number): string {
        const timeNow = new Date().toLocaleTimeString(this.options.locale, { timeZone: this.options.timeZone });
